...
describe('TagInput', () => {
  it('renders one chip per item', () => {
    const { getByText } = render(() => <TagInput items={['.rvz', '.cso']} onChange={() => {}} />)
  ...
  it('commits the draft as a chip when a comma is typed', () => {
    ...
    const { getByRole } = render(() => <TagInput items={items()} onChange={setItems} />)
  ...
  it('removes a chip when its X is clicked', () => {
    const fn = vi.fn()
    const { getByLabelText } = render(() => <TagInput items={['.rvz', '.cso']} onChange={fn} />)
  ...
  it('does not remove the last chip via Backspace when disabled', () => {
    const fn = vi.fn()
    const { getByRole } = render(() => <TagInput items={['.rvz']} onChange={fn} disabled />)
