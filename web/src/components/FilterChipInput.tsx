...
export const FilterChipInput: Component<FilterChipInputProps> = (props) => {
  ...
  const appendChips = (incoming: FilterChip[]) => {
    ...
    const seen = new Set(
      props.chips.map((c) => `${c.negative ? "-" : "+"}${c.text.toLowerCase()}`),
    );
    ...
    <div class="row" style={{ "margin-bottom": "8px" }}>
      ...
      <div class="filter-chip-input" role="group" style={{ flex: 1 }}>
        <For each={props.chips}>
          {(chip, index) => (
            <span
              class={`filter-chip is-${chip.negative ? "negative" : "positive"}`}
            >
        ...
        <input
          ...
          value={draft()}
          placeholder={
            props.chips.length === 0 ? props.placeholder ?? "type to filter..." : ""
          }
          onInput={onInput}
          onKeyDown={onKeyDown}
          onBlur={onBlur}
