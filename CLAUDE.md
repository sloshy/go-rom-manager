...
- **`games/`** — pure functions, no I/O. `ParseName` splits filename → prefix+tags. `GroupFiles` buckets files by prefix, honouring a `manualGroups` map (filename → override-prefix) for cases like merging a Japanese release into the same group as its Western counterpart. `AutoSelectWith(files, preferences, lowPriorityTags)` picks the best variant of a group: drop any file carrying a low-priority tag if a cleaner one exists → walk `preferences` top-to-bottom, first matching tag wins → break ties by highest `Rev N`. `AutoSelect(files)` is a thin wrapper using `DefaultPreferences = ["USA", "World"]` and `DefaultLowPriorityTags = ["Demo", "Proto", "Sample"]`. The frontend mirrors this logic in `web/src/lib/games.ts` (`DEFAULT_LOW_PRIORITY_TAGS` ↔ `DefaultLowPriorityTags`); **keep the two implementations in sync** when changing selection rules. The Rev tiebreaker is fixed; both the priority list and the low-priority list are user-configurable (global + per-mapping override). An empty low-priority list demotes nothing; when every variant is low-priority the filter is skipped so a Demo/Sample-only game still resolves.
...
- **`config/`** — `AppConfig` is the validated CLI args. `Store` is a mutex-guarded JSON file of `Mapping{ID, Name, SourcePaths, PrimarySource, DestPath, ManualGroups, Preferences, LowPriorityTags, AllowedExtensions, ExtractArchives}` plus top-level `globalPreferences` and `globalLowPriorityTags`. All writes are temp+rename. `Mapping.SourcePaths` is the ordered list of source folders feeding the destination; `PrimarySource` (omitted when empty) names the one that loads first, else the first entry is used. The legacy single `sourcePath` field is migrated into `SourcePaths` on load and cleared (so it never round-trips). `Mapping.Preferences` and `Mapping.LowPriorityTags` are both `*[]string`: `nil` means "inherit the global list"; a non-nil slice (even empty) is an explicit override. `globalPreferences`/`globalLowPriorityTags` are themselves `*[]string` so an unset field falls back to `games.DefaultPreferences` / `games.DefaultLowPriorityTags`. `Mapping.AllowedExtensions` is a normalized list of lowercase ".ext" entries; empty means strict-name matching. **Note**: there is intentionally no `Selections` field — the set of "selected" files is derived from disk at runtime, not persisted. Legacy `selections` keys in older mappings.json files are silently dropped on the next save (`encoding/json` ignores unknown fields by default).
- **`server/`** — `http.ServeMux` with method-pattern routes (Go 1.22+ syntax, e.g. `GET /api/mappings/{id}`). `spaFileServer` serves the embedded `web/dist`, falling through to `index.html` for unknown paths so client-side routes resolve. **`/api/*` paths never fall through to the SPA** — they 404 explicitly. `GET /api/mappings/{id}` returns a per-source `sources: [{path, files, groups}]` array (configured order) plus `destFiles`, `effectivePreferences`, and `effectiveLowPriorityTags`. Both `effective*` lists (per-mapping override → global → default) are resolved server-side so the frontend never has to merge them. `cleanPreferences` trims/drops blanks and rejects case-insensitive duplicates on every preferences PUT (settings or per-mapping). `cleanTags` does the same for the low-priority list but *silently* dedupes case-insensitively rather than erroring (it's an unordered set). The global `PUT /api/settings` payload has pointer fields (`preferences`, `lowPriorityTags`) so each list can be saved independently; an omitted field is left unchanged. `GET /api/settings` and the PUT response both report `preferences`/`defaultPreferences`/`lowPriorityTags`/`defaultLowPriorityTags`. The per-mapping low-priority override has its own endpoint `PUT /api/mappings/{id}/low-priority-tags` (mirror of `/preferences`): a non-null `lowPriorityTags` array sets an override, `null` clears it back to inheriting global. `cleanSourcePaths` (on `PUT /api/mappings/{id}`) trims/dedupes the source list, requires ≥1, and runs each through `EnsureUnderRoot`; `resolvePrimarySource` validates the optional primary is one of them. `cleanExtensions` normalizes `allowedExtensions` to lowercase ".ext" form and silently dedupes. `POST /api/mappings/{id}/sync` takes `intended: [{name, dir}]`; each `dir` is validated to be one of the mapping's configured sources (both to fix the copy origin and to refuse copies from arbitrary directories).
...
Routes (`App.tsx`): `/` Home, `/mapping/:id` editor, `/settings` global preferences (AUTO-SELECT PRIORITY panel + LOW PRIORITY TAGS panel, each with its own save button), `/mapping/:id/settings` per-mapping preference override + per-mapping low-priority override (each with an "inheriting" preview when no override is set) plus the EDIT MAPPING panel (name + the multi-source manager — add/remove sources, mark one primary — + destination + `allowedExtensions`). The editor's source column title bar hosts the `SourceDropdown` switcher; `lib/paths.ts#sourceLabel` gives the short per-source display label used there and in the dest subtext. The preference panels share `components/PreferenceEditor.tsx`, a controlled drag-and-drop list (priority order matters) — the parent owns the array, the parent renders the green save button (`tui-button--save`). The low-priority panels use `components/TagInput.tsx` (an unordered set) instead. `TagInput` is the comma-commits / X-removes chip editor also used for `allowedExtensions`; it accepts an optional `normalize` callback (e.g. lowercase + ensure leading dot) and silently dedupes.
