...
- **`fsutil/`** — filesystem operations. `EnsureUnderRoot` is the single chokepoint that prevents path traversal; every user-supplied path goes through it before any read/write. `ComputeSync(intended, sourceFiles, destDir, allowedExts)` produces a `SyncPlan{ToCopy, ToDelete}`; `ExecuteSync` applies it (atomic copy via temp+rename, delete leftovers). `allowedExts` lets a dest file (e.g. `Game.rvz`) match a source file by stem (`Game.zip`) when its extension is on the list — see "alt-extension matching" below. `Browse`/`ListFiles` are sorted dirs-first / case-insensitive.
...
2. Frontend derives the initial selected set as **`destFiles ∩ sourceFiles`** (with alt-ext matching, see below): a source file is "selected" iff dest contains either (a) the same filename or (b) a file with the same stem and an extension in `allowedExtensions`.
3. Files in `destFiles` with no source counterpart (by exact name AND by alt-ext stem match) are the **orange extras** — they have nowhere to come from, so the mapping cannot manage them. The UI renders them disabled.
...
5. POST `/api/mappings/{id}/sync` sends `{ intended: string[], manualGroups }`. Server computes `ComputeSync(intended, sourceFiles, destDir, m.AllowedExtensions)`:
   - **Copy**: anything in `intended` missing from dest *and* not already represented by an alt-ext file in dest with the same stem.
   - **Delete**: anything in dest that's not satisfying any intent (exact or alt-ext) **and** is "managed" — i.e. has a source counterpart by exact name, or has an extension in `allowedExtensions` and shares a stem with a source file. No-source-counterpart files are never deleted — that's how orange survives.
...
**Alt-extension matching.** When `allowedExtensions` is non-empty, sync treats `Game.rvz` in dest as the same file as `Game.zip` in source if `.rvz` is on the list — useful when an external tool converts ROMs to a different on-disk format. Match key: same `filepath.Ext`-stripped stem AND dest extension is in the (lowercase, leading-dot) allowed list. Source extensions are not constrained. The frontend mirrors this in `web/src/lib/games.ts` (`fileStem`, `isAllowedExt`) and `web/src/stores/editor.ts` (every memo: `deriveInitialSelected`, `extraFiles`, `filesToRemove`, `pendingDiff`); **keep frontend and backend in sync** when changing the matching rule.
...
- **`stores/editor.ts`** — single mapping being edited. On `load()` the initial `selected: Record<filename, true>` set is derived from `destFiles ∩ sourceFiles` (exact filenames, plus alt-ext stem matches when `allowedExtensions` is set); `manualGroups` rehydrates from the persisted mapping. Both stage in memory until `sync()`; `dirty` tracks whether anything changed. The store exposes derived memos for what the destination *will* look like (`destProjectionGroups`), what's queued for deletion (`filesToRemove` = deselected dest files that are "managed" — exact-name or alt-ext source match), what's untouched (`extraFiles` = dest files with no source counterpart by either path), and pending op counts (`pendingDiff`). UI colour coding (green/red/orange) flows from these memos. `togglePrefix` calls `autoSelect(files, detail.effectivePreferences)` — the editor never resolves override-vs-global itself.
